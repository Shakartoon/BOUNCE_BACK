...
public class audioVisualizerSineWave : MonoBehaviour {
	...
	void Start () {
		...
		plane.GetComponent<Renderer>().material = colourMat;
	}
	...
	void OnGUI() {
		...
		if (plane.GetComponent<Renderer>().material.name.StartsWith(colourMat.name)) {
			if (GUI.Button(buttonCoords, "Switch to Wireframe render")) {
				plane.GetComponent<Renderer>().material = wireframeMat;
			}
		} else {
	...
	void setNewVertexPos(ref Vector3[] vertices, ref Color[] colors, ref float[] dampingVelocities, ref float decrement, ref int index, int iterations, float factor, float amp, float smoothTime, int x, int a) {
		...
		colors[widthVerts * x + a].r = Mathf.Abs(vertices[widthVerts * x + a].y) / sineScale(0.4f, 4, 5f, 12f) * Time.deltaTime; //8
		colors[widthVerts * x + a].g = Mathf.Abs(vertices[widthVerts * x + a].y) / sineScale(0.1f, -2, 9f, 15f) * Time.deltaTime; //10
		colors[widthVerts * x + a].b = Mathf.Abs(vertices[widthVerts * x + a].y) / sineScale(0.05f, 0, 2f, 8f) * Time.deltaTime; //2
